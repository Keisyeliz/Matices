...
def fnt_limpiar():
    ...
    print('Semestre: I')
    fnt_impresion_abecedario()
    print('\n')

def fnt_impresion_abecedario():
    for i in range(len(abecedario)):
        for j in range(len(abecedario[i])):
            print(abecedario[i][j], end = ' ')
        print()
def fnt_agregar(dato,x,y):
    if dato == 'a' and x == 0 and y == 0:
        abecedario[x][y] = dato.upper()
        ...
        enter = input('\nEste no es el lugar que le corresponde a esta letra <ENTER>')
